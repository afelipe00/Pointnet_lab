Public Class Class_add_compra
    Public Function espacios_vacios(ByRef lista As ListView, ByRef combox As ComboBox, ByRef cantidad As TextBox, ByRef nom As TextBox,
                                    ByRef ref As TextBox, ByRef Descripcion As RichTextBox, ByRef fabricante As TextBox, ByRef costu As TextBox, ByRef costt As TextBox, ByRef garantia As DateTimePicker,
    ...
    Public Function Comprobar_compra(ByRef codigo As TextBox, ByRef fecha As DateTimePicker, ByRef proveedor As ComboBox, ByRef representante As TextBox,
                                     ByRef id As TextBox, ByRef correo As TextBox, ByRef lista As ListView, ByRef total As TextBox,
                                     ByRef label1 As Label, ByVal hoja As String)
        Dim funciones = New Class_funciones
        Dim workbook = Globals.ThisWorkbook.Application
        Dim vector() As Object = {proveedor, representante, id, correo, lista, total}
        ...
        Next
        If estado = 0 Then
            Agregar_compra(codigo, fecha, proveedor, lista, hoja)
            Dim fil_celular As Long = funciones.repeat_fila(proveedor.Text, "Proveedores", 2, 1, 1)
            Dim celular As Long = workbook.Sheets("Proveedores").cells(fil_celular, 3).value
            funciones.Llenar_Factura("Form. Facturas", codigo.Text, fecha.Text, proveedor.Text, id.Text, correo.Text, celular, "0", lista)
    ...
    Public Function Agregar_compra(ByRef codigo As TextBox, ByRef fecha As DateTimePicker, ByRef proveedor As ComboBox, ByRef lista As ListView,
    ...
    Public Function cost_t(ByRef cantidad As TextBox, ByRef costo_total As TextBox, ByRef costo_u As TextBox, ByRef label As Label)
        Dim funcion = New Class_funciones
    ...
    Public Function Agregar_inventarios(ByRef lista As ListView, ByVal hoja As String, ByVal colum_codigo As Integer, ByVal colum_cantidad As Integer)
        ...
        For i = 0 To lista.Items.Count - 1
            ...
            If fila_rep <> 1 Then
                workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad).value = cantidad(i) + workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad).value
            ...
            Else
                'creando esos valores en la hoja de inventario
                fila_rep = funciones.repeat_fila(codigos(i), "Productos", 2, colum_codigo, 1)
                With workbook.Sheets(hoja)
                    .Cells(fila_final, 1).value = workbook.Sheets("Productos").cells(fila_rep, 1).value
                    .Cells(fila_final, 2).value = workbook.Sheets("Productos").cells(fila_rep, 2).value
                    .Cells(fila_final, 3).value = workbook.Sheets("Productos").cells(fila_rep, 3).value
                    .Cells(fila_final, 4).value = workbook.Sheets("Productos").cells(fila_rep, 4).value
                    .Cells(fila_final, 5).value = workbook.Sheets("Productos").cells(fila_rep, 5).value
                    .Cells(fila_final, 7).value = workbook.Sheets("Productos").cells(fila_rep, 6).value
                    .Cells(fila_final, 8).value = workbook.Sheets("Productos").cells(fila_rep, 7).value
                End With
                fila_rep = funciones.repeat_fila(codigos(i), hoja, 2, colum_codigo, 1)
                workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad).value = cantidad(i) + workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad).value
                workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad + 3).value = workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad).value * workbook.Sheets(hoja).Cells(fila_rep, colum_cantidad + 1).value
